  ...
  return new Promise((resolve, reject) => {
    ...
    request(url).then($ => {
      ...
      for (let i = 1; i < _length; i++) {
        novelList.push({
          // 编号
          'index': clearfix.eq(i).children('.col-xs-3').children('a').attr('href').split('/')[2],
          // 类型: types
          'types': clearfix.eq(i).children('.col-xs-1').children('.tag-blue').html(),
          // 小说名称:novel
          'novel_list': 'https://www.boquge.com/book/' + clearfix.eq(i).children('.col-xs-3').children('a').attr('href').split('/')[2],
