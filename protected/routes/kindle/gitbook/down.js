      ...
      if (mkdirName) {
        ...
        function down() {
          let time = setTimeout(() => {
            if (i === list.length) {
              clearInterval(time);
              fs.writeFile(`${path}${params.index}.json`, JSON.stringify(books), function (err) {
                if (err) {
                  console.error(err);
                } else {
                  shell.exec(`cd ${path} && gitbook mobi ./ ./${params.index}.mobi`);
                }
              });
              resolve('下载完毕');
            } else if (list[i]) {
...
function mkdir(name) {
  return new Promise((resolve, reject) => {
    ...
    // 删除原有文件
    shell.exec(`rm -rf ./gitbook/${name} ./gitbook/${name}/page && mkdir ./gitbook/${name} ./gitbook/${name}/page`);
    resolve(true)
  })
}